...
export const SdkDocumentation: React.FC<SdkDocumentationProps> = ({
  ...
  const markdown = !frontmatterMatch
    ? normalizedMarkdown
    : normalizedMarkdown.slice(frontmatterMatch[0].length).trim();
  ...
  // 🚀 每次渲染都重新生成标题 ID，避免 useMemo 缓存导致计数器跨重渲染累积，
  // 🚀 使目录 href 与标题真实 id 发生错位，出现首次点击目录不跳转的问题。
  const renderHeading =
    (level: 1 | 2 | 3 | 4 | 5 | 6) =>
    ({ children, ...props }: { children?: React.ReactNode }) => {
      ...
      return (
        <TagName
          ...
          {...props}
          //@ts-ignore
          className={classNames([styles['sdk-heading']], props.className)}
        >
          {children}
        </TagName>
      );
