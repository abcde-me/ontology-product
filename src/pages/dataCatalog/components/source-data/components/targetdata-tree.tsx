...
// 转换原始数据为Tree组件格式
const rawTreeData = FlaseTreeData.dst;
...
export default function SourceDataTree(props: SourceDataTreeProps) {
  ...
  const [addingToParentInfo, setAddingToParentInfo] = useState<string | null>(
    null
  ); // 记录正在为哪个父节点添加子节点（存储父节点的key）
  ...
  // 监听activeTab变化，当切换到target时重新设置默认选中状态
  useEffect(() => {
    if (activeTab === 'target') {
      console.log('切换到目标数据，重新设置默认选中状态');
      // 重新设置默认展开和选中状态
      setExpandedKeys(['0-1', '0-1-volume', '0-1-db']);
      setSelectedKey('0-1-volume-10');
      ...
      const defaultNodeInfo = getDefaultSelectedNodeInfo();
      if (defaultNodeInfo) {
        console.log('目标数据 - 默认选中节点信息:', defaultNodeInfo);
      ...
      const updateNodeName = (
        ...
        return nodes.map((node) => {
          ...
          if (node.children) {
            return {
              ...node,
              children: updateNodeName(node.children, targetKey, newName)
            };
          }
          return node;
        });
        ...
        {treeData && treeData.length > 0 ? (
          <Tree
                        ...
                        <>
                          ...
                          <Tooltip color="white" content="删除">
                            <IconDelete
                              ...
                              onMouseEnter={() =>
                                setHoverDeleteKey(node._key || null)
                              }
                          ...
                          {(node.title == '数据库' ||
                            ...
                            <IconPlus
                              ...
                              onClick={() => {
                                ...
                                if (
                                  node._key &&
                                  !expandedKeys.includes(node._key)
                                ) {
                          ...
                          {node.dataRef?.rawData?.type === 'catalog' && (
                            <Tooltip color="white" content="编辑">
                              <IconEdit
                                ...
                                onMouseEnter={() =>
                                  setHoverEditKey(node._key || null)
                                }
                                onMouseLeave={() => setHoverEditKey(null)}
                                onClick={() => {
                                  // 点击编辑图标时开始编辑节点
                                  const nodeName =
                                    node.dataRef?.rawData?.name ||
                                    node.title ||
                                    '';
            ...
            renderTitle={(props) => {
              ...
              const hasChildren =
                props.dataRef && props.dataRef.children !== undefined;
                ...
                return (
                  <Input
                    ...
                    onPressEnter={() => {
                      ...
                      // 如果输入为空，使用默认名称
                      const finalName =
                        newNodeValue.trim() || `新建目录${Date.now()}`;
