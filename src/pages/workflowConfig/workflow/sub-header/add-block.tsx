...
const AddBlock = ({
  ...
  const handleSelect = useCallback<OnSelectBlock>((type, toolDefaultValue) => {
    ...
    const { newNode } = generateNewNode({
      data: {
        ...
        title: nodesWithSameType.length > 0 ? `${t(`workflow.blocks.${type}`)}_${nodesWithSameType.length + 1}` : t(`workflow.blocks.${type}`),
        ...(toolDefaultValue || {}),
        _isCandidate: true,
      },
      position: {
  ...
  const renderTriggerElement = useCallback((open: boolean) => {
    return (
      ...
      >
        <div className={`add-node-btn ${nodesReadOnly ? 'cursor-not-allowed' : ''}`}>
