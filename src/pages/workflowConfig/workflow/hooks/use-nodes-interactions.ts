  ...
  const handleNodeConnect = useCallback<OnConnect>(
    ({ source, sourceHandle, target, targetHandle }) => {
      ...
      const newEdge = {
        ...
        zIndex: targetNode?.parentId ? LOOP_CHILDREN_Z_INDEX : 0
      };
    ...
    (
          ...
          data: {
            ...
            title:
              nodesWithSameType.length > 0
                ? `${t(`workflow.blocks.${nodeType}`)}_${nodesWithSameType.length + 1}`
      ...
      if (prevNodeId && !nextNodeId) {
        ...
        const newEdge: Edge = {
          ...
          markerEnd,
          zIndex: prevNode.parentId ? LOOP_CHILDREN_Z_INDEX : 0
        };
        const nodesConnectedSourceOrTargetHandleIdsMap =
          getNodesConnectedSourceOrTargetHandleIdsMap(
            [{ type: 'add', edge: newEdge }],
      ...
      if (!prevNodeId && nextNodeId) {
        ...
        const newEdge = {
          ...
          markerEnd,
          zIndex: nextNode.parentId ? LOOP_CHILDREN_Z_INDEX : 0
        };
