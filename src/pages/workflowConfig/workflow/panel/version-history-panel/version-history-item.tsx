...
const VersionHistoryItem: React.FC<VersionHistoryItemProps> = ({
  ...
  return (
    <div
      className={cn(
        'flex gap-x-1 relative p-2 rounded-[6px] group',
      ...
      // }}
    >
      {!isLast && <div className='hidden absolute w-0.5 h-[calc(100%-0.75rem)] left-4 top-6 bg-divider-subtle' />}
      <div className=' flex items-center justify-center shrink-0 w-[18px] h-5'>
        <div className={cn(
          'w-2 h-2 border-[2px] rounded-lg',
          isSelected ? 'border-[#007DFA]' : 'border-[#94A3B8]',
        )}/>
      </div>
      <div className='flex flex-col gap-y-0.5 grow overflow-hidden'>
        <div className='flex items-center gap-x-1 h-5 mr-6'>
          <div className={cn(
            'py-[1px] system-sm-semibold truncate text-[14px]/[20px]',
            isSelected ? 'text-[#007DFA]' : 'text-[#1E293B]',
          )}>
            {isDraft ? t('workflow.versionHistory.currentDraft') : item.marked_name || t('workflow.versionHistory.defaultName')}
          </div>
          {isLatest && (
            <div className='flex items-center shrink-0 h-5 px-[5px] rounded-[4px] border border-text-accent-secondary
        ...
        {
          !isDraft && (
            <div className='text-[#6E7B8D] system-xs-regular truncate'>
