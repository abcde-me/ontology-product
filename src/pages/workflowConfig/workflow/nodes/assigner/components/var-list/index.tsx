...
import ListNoDataPlaceholder from '@/pages/workflowConfig/workflow/nodes/_base/components/list-no-data-placeholder';
            ...
            <div className="flex grow flex-col items-start gap-1">
              <div className="flex items-center gap-1 self-stretch">
                <VarReferencePicker
                  ...
                  filterVar={filterVar}
                  placeholder={t(
                    'workflow.nodes.assigner.selectAssignedVariable'
                  )}
                  minWidth={352}
                  popupFor="assigned"
                  className="w-full"
                ...
                <OperationSelector
                  ...
                  disabled={
                    !item.variable_selector ||
                    item.variable_selector.length === 0
                  }
              ...
              {item.operation !== WriteMode.clear &&
                ...
                !writeModeTypesNum?.includes(item.operation) && (
                  <VarReferencePicker
                    readonly={
                      readonly || !item.variable_selector || !item.operation
                    }
                    nodeId={nodeId}
                    isShowNodeName
                    value={item.value}
                ...
                <>
                  {assignedVarType === 'number' && (
                    <Input
                      ...
                      onChange={(e) =>
                        handleToAssignedVarChange(index)(Number(e.target.value))
                      }
                      className="w-full"
                    />
                  )}
                  {assignedVarType === 'string' && (
                    <Textarea
                      ...
                      onChange={(e) =>
                        handleToAssignedVarChange(index)(e.target.value)
                      }
                      className="w-full"
                    />
                  )}
                  {assignedVarType === 'object' && (
                    <CodeEditor
                      ...
                      onChange={(value) =>
                        handleToAssignedVarChange(index)(value)
                      }
                      className="w-full"
                      readOnly={readonly}
                    />
