  ...
  const handleListChange = useCallback(
    (newList: ValueSelector[], changedItem?: ValueSelector) => {
      if (changedItem) {
        ...
        if (
          chosenVariables.some(
            (item) => item.join('.') === changedItem.join('.')
          )
        )
          return;
      }
        ...
        groupEnabled ? (
          <div className="flex items-center">
            <div className="flex items-center !normal-case">
              <RiFolder2Line className="mr-0.5 h-3.5 w-3.5" />
              {!isEditGroupName ? (
                <div
                  className="system-sm-semibold flex h-6 cursor-text items-center rounded-lg px-1 text-text-secondary hover:bg-gray-100"
      ...
      operations={
        <div className="flex h-6 items-center  space-x-2">
          {payload.variables.length > 0 && (
            <div className="system-2xs-medium-uppercase flex h-[18px] items-center rounded-[5px] border border-divider-deep px-1 text-text-tertiary">
              {payload.output_type}
            </div>
          )}
          {!readOnly ? (
