...
const NodeVariableItem = ({
  ...
  return (
    <div className={cn(
      'relative flex items-center p-[3px] pl-[5px] self-stretch rounded-[4px] bg-workflow-block-parma-bg item-bg mb-[4px]',
      ...
      <div className='flex items-center text-primary-600 w-full'>
        ...
        {!isChatVar && <div className={cn('max-w-[75px] truncate ml-0.5 system-xs-medium overflow-hidden text-ellipsis name', isEnv && 'text-gray-900', isException && 'text-text-warning')} title={varName}>{varName}</div>}
        {isChatVar
