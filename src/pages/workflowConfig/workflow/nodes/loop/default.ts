...
const nodeDefault: NodeDefault<LoopNodeType> = {
  ...
  checkValid(payload: LoopNodeType, t: any) {
    ...
    if (
      !errorMessages &&
      (!payload.break_conditions || payload.break_conditions.length === 0)
    )
      errorMessages = t(`${i18nPrefix}.fieldRequired`, {
        field: t('workflow.nodes.loop.breakCondition')
      });
    ...
    payload.break_conditions!.forEach((condition) => {
      ...
      if (!errorMessages) {
        if (
          ...
          const isSet = condition.sub_variable_condition.conditions.every(
            (c) => {
