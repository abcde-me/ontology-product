          ...
          draft.forEach((n) => {
            if (n.id === nodeId) {
              if (widthShouldExtend) {
                n.data.width =
                  rightNode.position.x + rightNode.width! + LOOP_PADDING.right;
                n.width =
                  rightNode.position.x + rightNode.width! + LOOP_PADDING.right;
              }
      ...
      if (node.data.isInLoop) {
        ...
        if (parentNode) {
          ...
          if (
            ...
            parentNode.width! - LOOP_PADDING.right
          )
            restrictPosition.x =
              parentNode.width! - LOOP_PADDING.right - node.width!;
          if (
            node.position.y + node.height! >
            parentNode.height! - LOOP_PADDING.bottom
          )
            restrictPosition.y =
              parentNode.height! - LOOP_PADDING.bottom - node.height!;
        }
      }
  ...
  const handleNodeLoopChildrenCopy = useCallback(
    (nodeId: string, newNodeId: string) => {
      ...
      const childrenNodes = nodes.filter(
        (n) => n.parentId === nodeId && n.type !== CUSTOM_LOOP_START_NODE
      );
