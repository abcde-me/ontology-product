...
import { PortalSelect } from '@/pages/workflowConfig/components/select';
...
const KeyValueItem: FC<Props> = ({
  ...
  return (
    ...
    <div
      className={cn(className, 'group flex h-[32px] items-center gap-x-[8px]')}
    >
      <div className={cn('h-full', isSupportFile ? 'w-[140px]' : 'w-1/2')}>
        {!keyNotSupportVar ? (
          <InputItem
      ...
      {isSupportFile && (
        <div className="w-[70px] shrink-0 border-r border-divider-regular">
          <PortalSelect
            ...
            triggerClassNameFn={(isOpen) =>
              isOpen ? 'bg-state-base-hover' : 'bg-transparent'
            }
            popupClassName="w-[80px] h-7"
          />
        </div>
      )}
      <div className={cn('h-full', isSupportFile ? 'grow' : 'w-1/2')}>
