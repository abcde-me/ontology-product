...
import RemoveButton from '@/pages/workflowConfig/workflow/nodes/_base/components/remove-button'
      ...
      {(!readOnly)
        ? (
          <Input
            ...
            className={cn(isFocus ? 'bg-gray-100' : 'bg-width', 'w-0 grow px-3 py-[6px] !pt-[5px] outline-input overflow-y-hidden')}
            value={value}
            onChange={onChange}
            readOnly={readOnly}
        ...
        >
          ...
          {hasValue && (
            <Input
              instanceId={instanceId}
              className={cn(isFocus ? 'shadow-xs bg-gray-50 border-gray-300' : 'bg-gray-100 border-gray-100', 'w-0 grow rounded-[4px] px-3 py-[6px] border')}
              value={value}
              onChange={onChange}
              readOnly={readOnly}
              nodesOutputVars={availableVars}
