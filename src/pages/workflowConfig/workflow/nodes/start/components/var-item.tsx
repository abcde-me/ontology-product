...
import type { InputVar, MoreInfo } from '@/pages/workflowConfig/workflow/types'
...
const VarItem: FC<Props> = ({
  ...
  return (
    <div ref={ref} className='flex items-center h-8 justify-between px-2.5 bg-white rounded-[4px] border border-gray-200 shadow-xs cursor-pointer hover:shadow-md'>
          ...
          {(!isHovering || readonly)
            ? (
              <>
                ...
                <InputVarTypeIcon isTag type={payload.type} className='w-3.5 h-3.5 text-gray-500' />
              </>
            )
            : (!readonly && (
              <>
                <div onClick={showEditVarModal} className='mr-1 p-1 rounded-md cursor-pointer hover:bg-black/5'>
                  <RiEdit2Line className='w-4 h-4 text-gray-500' />
                </div>
