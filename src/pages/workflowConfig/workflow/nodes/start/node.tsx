...
const Node: FC<NodeProps<StartNodeType>> = ({
  ...
  const hasFileTypes = data_category?.[0]?.enabled || data_category?.[1]?.enabled
    || data_category?.[2]?.enabled || data_category?.[3]?.enabled
    ...
    <div className={`wk-node-content`}>
      <div className={`input-section`}>
        ...
        <div className='input-list'>
          ...
          {!source_path && <div className='input-var-item'><span className='extra-info'>未配置</span></div>}
        </div>
      </div>
      <div className={`input-section`}>
        <div className='input-header'>
        ...
        <div className='input-list'>
          {!!hasFileTypes &&
            <div className='input-var-item flex gap-x-[4px] !justify-normal *:rounded-[4px] *:px-[4px] *:bg-[#E2E8F0] *:text-[12px]/[18px] *:text-[#0F172A]'>
              ...
              {data_category?.[1]?.enabled && data_category?.[1]?.format.length > 0 && <div>图片</div>}
              {data_category?.[2]?.enabled && data_category?.[2]?.format.length > 0 && <div>音频</div>}
              {data_category?.[3]?.enabled && data_category?.[3]?.format.length > 0 && <div>视频</div>}
            </div>
          }
