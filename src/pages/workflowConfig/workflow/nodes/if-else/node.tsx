...
import { LogicalOperator } from '../loop/types'
...
const IfElseNode: FC<NodeProps<IfElseNodeType>> = (props) => {
  ...
  const conditionNotSet = (<div className='key-txt-origin'>
    {t(`${i18nPrefix}.conditionNotSetup`)}
  </div>)

  const toggleVar = (idx: number) => {
    document.querySelector('.if-else-node .output-section.case-' + idx).classList.toggle('collapsed')
  }

  return (
    // <div className='px-3 wk-node-content'>
    //   {
    ...
    //             <div key={condition.id} className='relative item-bg'>
    //               {
    ...
    //   </div>
    // </div>
    <div className='wk-node-content if-else-node'>
      {
        cases.map((caseItem, index) => (
          <div key={caseItem.case_id} className={`output-section relative case-${index}`}>
            <div className='relative output-header'>
              <span className='txt'>{index === 0 ? '如果' : '否则如果'}</span>
              {!!caseItem.conditions.length && <RiArrowDownSFill className='icon cursor-pointer' onClick={() => toggleVar(index)}/>}
            </div>
            <div className='output-list'>
              {caseItem.conditions.map((condition, i) => (
                <div key={condition.id} className='relative output-var-item'>
                  {
                    checkIsConditionSet(condition)
                      ? (
                        (!isEmptyRelatedOperator(condition.comparison_operator!) && condition.sub_variable_condition)
                      ...
                      : conditionNotSet}
                  {i !== caseItem.conditions.length - 1 && (
                    <div className='logic-operator-separator'><span className='z-[1]'>{caseItem.logical_operator === LogicalOperator.and ? '且' : '或'}</span></div>
                  )}
                </div>
              ))}
            </div>
      ...
      <div className='relative input-section'>
        <div className='input-header'>否则</div>
        <NodeSourceHandle
          {...props}
          handleId='false'
          handleClassName='!top-1/2 !-right-[21px] !-translate-y-1/2'
