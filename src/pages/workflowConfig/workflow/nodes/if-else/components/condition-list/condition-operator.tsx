...
import { getOperators, getOperatorsByType, isComparisonOperatorNeedTranslate } from '../../utils'
...
const ConditionOperator = ({
  ...
  const options = useMemo(() => {
    return getOperatorsByType(varType).map((o) => {
      return {
        label: o[0],
        value: o[1] as ComparisonOperator,
      }
    })
  }, [varType])
  ...
  return (
    ...
    >
      ...
      <PortalToFollowElemContent className='z-10'>
        <div className='p-1 bg-components-panel-bg-blur rounded-[4px] border-[0.5px] border-components-panel-border shadow-lg'>
          {
            options.map(option => (
              <div
                key={option.value}
