...
import { SimpleSelect as Select } from '@/pages/workflowConfig/components/select'
...
const ConditionItem = ({
  ...
  const isBooleanTrueFalse = condition.comparison_operator && [ComparisonOperator.isFalse, ComparisonOperator.isTrue].includes(condition.comparison_operator)
  const isEmptyOrNot = condition.comparison_operator && [ComparisonOperator.empty, ComparisonOperator.notEmpty].includes(condition.comparison_operator)
  ...
  const isNotInput = isSelect || isSubVariable || isBooleanTrueFalse || isEmptyOrNot
  ...
  return (
    <div className={cn('flex mb-[12px] last-of-type:mb-0 condition-item', className)}>
      <div className={cn(
        'grow',
      )}>
        <div className='flex items-center condition-item-var-operator bordered'>
                  ...
                  renderTrigger={item => (
                    item
                      ? <div className='flex justify-start cursor-pointer'>
                        <div className='inline-flex max-w-full px-1.5 items-center h-6 rounded-[4px] border-[0.5px] border-components-panel-border-subtle bg-components-badge-white-to-dark shadow-xs text-text-accent'>
                          <RiBracesLine className='shrink-0 w-3.5 h-3.5 text-text-accent' />
          ...
          <ConditionOperator
            ...
            file={fileAttr}
            className="condition-operator-btn"
          />
        </div>
        {
          !comparisonOperatorNotRequireValue(condition.comparison_operator) && !isNotInput && (
            <div className='condition-item-input'>
              <ConditionInput
                disabled={disabled}
        ...
        {/* {
          !comparisonOperatorNotRequireValue(condition.comparison_operator) && !isNotInput && condition.varType === VarType.number && (
            <div className='px-2 py-1 pt-[3px] border-t border-t-divider-subtle condition-item-input'>
        ...
        }
        {
          !comparisonOperatorNotRequireValue(condition.comparison_operator) && isSelect && (
            <div className='border-t border-t-divider-subtle condition-item-input'>
              <Select
        ...
        } */}
        {
          !comparisonOperatorNotRequireValue(condition.comparison_operator) && isSubVariable && (
            <div className='p-1 condition-item-sub-var'>
              <ConditionWrap
                isSubVariable
                caseId={caseId}
      ...
      <div
        className='shrink-0 flex items-center justify-center ml-1 mt-1 w-6 h-auto cursor-pointer text-text-tertiary'
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        onClick={doRemoveCondition}
      >
        {/* <RiDeleteBinLine className='w-4 h-4' /> */}
        <IconMinusCircle className='size-[16px]' />
      </div>
    </div>
  )
}
