...
import { PortalSelect as Select } from '@/pages/workflowConfig/components/select'
      ...
      >
        {
          cases.map((item, index) => (
            <div key={item.case_id} className="case-wrapper">
              <div
                className={cn(
                  'group relative rounded-[4px] bg-components-panel-bg',
                  willDeleteCaseId === item.case_id && 'bg-state-destructive-hover',
                  !isSubVariable && 'py-1 px-3 min-h-[40px] ',
                  isSubVariable && 'px-1 py-2',
                  'case-section bordered'
                )}
              >
                {/* {!isSubVariable && (
                  <>
...

                {
                  !!item.conditions.length && (
                    <div className='mb-2 conditoin-list-wrapper'>
                      <ConditionList
                        disabled={readOnly}
                        caseItem={item}
                ...
                <div className={cn(
                  ...
                  !item.conditions.length && !isSubVariable && 'mt-[12px]',
                  !item.conditions.length && isSubVariable && 'mt-2',
                  !isSubVariable && ' pl-[40px]',
                )}>
                  {isSubVariable
                    ? (
                      <Select
              ...
              {!isSubVariable && (
                <div className='my-[8px] mx-3 h-[1px] bg-divider-subtle bg-[transparent]'></div>
              )}
            </div>
          ))
        }
