...
import { RETRIEVE_TYPE } from '@/pages/workflowConfig/types/app'
import { DATASET_DEFAULT } from '@/pages/workflowConfig/config/'
...
import { useCurrentProviderAndModel, useModelListAndDefaultModelAndCurrentProviderAndModel } from '@/pages/workflowConfig/hooks/use-model'
import { ModelTypeEnum } from '@/pages/workflowConfig/models/model'
...
const useConfig = (id: string, payload: KnowledgeRetrievalNodeType) => {
  ...
  const handleretrievalModelChanged = useCallback((config: any) => {
    const newInputs = produce(inputRef.current, (draft) => {
      if (!draft.retrieval_model) {
        draft.retrieval_model = {
          search_method: 'hybrid_search', // semantic_search（语义检索） full_text_search（全文检索） hybrid_search（混合检索）
          ...
          score_threshold: 0.6 // 匹配分
        }
      } else {
        draft.retrieval_model = { ...config }
      }
    })
    setInputs(newInputs)
  }, [setInputs])
  ...
  useEffect(() => {
    (async () => {
      ...
      if (datasetIds?.length > 0) {
        const result = await getDatasetsList({page: 1, limit: Number.MAX_SAFE_INTEGER, ids: datasetIds.join(',')})
