...
import type { Node, NodeOutPutVar, ValueSelector, Var } from '@/pages/workflowConfig/workflow/types'
import type { CredentialFormSchema } from '@/pages/workflowConfig/models/model'
  ...
  return (
    <div className={cn(className, !readonly && 'cursor-pointer')}>
      ...
      >
        ...
        }} className='!flex group/picker-trigger-wrap relative var-reference-picker-wrapper'>
          <>
            {isAddBtnTrigger
              ? (
                <div>
              ...
              : (<div ref={!isSupportConstantValue ? triggerRef : null} className={cn('picker-input-wrapper', (open || isFocus) ? 'border-gray-300' : 'border-gray-100', 'relative group/wrap flex items-center w-full h-8', !isSupportConstantValue && 'p-[6px] px-[8px] rounded-[4px] bg-components-input-bg-normal', isInTable && 'bg-transparent border-none', readonly && 'bg-components-input-bg-disabled')}>
                  {isSupportConstantValue
                    ? <div onClick={(e) => {
                      e.stopPropagation()
                      setOpen(false)
                    ...
                    : (!hasValue && <div className='ml-1.5 mr-1 hidden'>
                      <RiBracesLine className={`w-4 h-4 ${readonly ? 'text-components-input-text-disabled' : 'text-components-input-text-placeholder'}`} />
                    </div>)}
                  {isConstant
                    ? (
                      <ConstantField
                      ...
                      >
                        <div ref={isSupportConstantValue ? triggerRef : null} className={cn('h-full', isSupportConstantValue && 'flex items-center pl-1 py-1 rounded-lg bg-gray-100')}>
                          <Tooltip popupContent={!isValidVar && hasValue && t('workflow.errorMsg.invalidVariable')}>
                            <div className={cn('h-full items-center px-[8px] rounded-[4px]', hasValue ? 'bg-white inline-flex border-[#CBD5E1] border-[1px]' : 'flex')}>
                              {hasValue
                                ? (
                                  <>
                                    {isShowNodeName && !isEnv && !isChatVar && (
                                      <div className='flex items-center'>
                                        ...
                                        <div className='mx-0.5 text-xs font-medium text-[#0F172A] truncate' title={outputVarNode?.title} style={{
                                    ...
                                    <div className='flex items-center font-medium text-[#007DFA]'>
                  ...
                  {(hasValue && !readonly && !isInTable) && (<div
                    className='invisible group-hover/wrap:visible absolute h-5 right-1 top-[50%] translate-y-[-50%] group p-1 rounded-full hover:bg-black/5 cursor-pointer'
