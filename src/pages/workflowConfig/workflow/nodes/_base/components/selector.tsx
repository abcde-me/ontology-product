    ...
    <div className={cn(!trigger && !noLeft && 'left-[-8px]', 'relative select-none', className)} ref={ref}>
      {trigger
        ...
        : (
          <div
            onClick={toggleShow}
            className={cn('flex items-center h-5 pl-1 pr-0.5 rounded-md text-xs font-semibold text-gray-700 cursor-pointer')}>
      ...
      {(showOption && !readonly) && (
        <div className={cn('absolute z-10 top-[24px] w-[120px] py-1 border border-gray-200 shadow-lg rounded-[4px] bg-white select-none', popupClassName)}>
          {list.map(item => (
            <div
              key={item.value}
              onClick={() => {
              ...
              className={cn(itemClassName, uppercase && 'uppercase', 'flex items-center h-[30px] justify-between min-w-[44px] px-3 rounded-none cursor-pointer text-[13px] font-medium text-gray-700 hover:bg-gray-50')}
            >
              <div>{item.label}</div>
              {showChecked && item.value === value && <RiCheckLine className='text-primary-600 w-4 h-4' />}
            </div>
          ))
          }
        </div>
