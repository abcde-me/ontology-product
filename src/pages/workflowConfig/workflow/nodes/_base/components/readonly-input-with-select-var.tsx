...
const ReadonlyInputWithSelectVar: FC<Props> = ({
  ...
  const res = (() => {
    ...
    const html: JSX.Element[] = strWithVarPlaceholder.split(VAR_PLACEHOLDER).map((str, index) => {
      ...
      return (<span key={index} className="from-var">
        <span className='relative top-[-3px] leading-[16px]'>{str}</span>
        <div className=' inline-flex h-[16px] items-center px-1.5 rounded-[4px] bg-white'>
          {!isEnv && !isChatVar && (
          ...
          <div className='flex items-center text-primary-600'>
            ...
            <div className={cn('max-w-[50px] ml-0.5 text-xs font-medium truncate name', (isEnv || isChatVar) && 'text-gray-900')} title={varName}>{varName}</div>
          </div>
        </div>
      </span>)
    })
