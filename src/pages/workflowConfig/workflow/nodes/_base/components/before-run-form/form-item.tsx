...
const FormItem: FC<Props> = ({
  ...
  return (
    <div className={cn(className)}>
      {!isArrayLikeType && (
        <div className="system-sm-semibold mb-[6px] flex h-[20px] items-center text-text-secondary">
          {!!payload.required && (
            <span className="size-[12px] text-center font-bold text-[rgb(245,63,63)]">
              *
            </span>
          )}
          <div className="mr-[12px] truncate text-[12px]/[16px] font-medium text-[#151B26]">
            {typeof payload.label === 'object'
              ? nodeKey
              : payload.variable || payload.label}
          </div>
          <div className="rounded-[4px] bg-[#F7F7F9] px-[8px] text-[12px]/[20px] text-[#6E7B8D]">
            {payload.type}
          </div>
      ...
      <div className="grow">
        {(type === InputVarType.textInput || type === InputVarType.string) && (
          <Input
            className="outline-input rounded-[4px]"
            value={value || ''}
