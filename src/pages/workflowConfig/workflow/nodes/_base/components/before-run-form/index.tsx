...
const BeforeRunForm: FC<BeforeRunFormProps> = ({
      ...
      form.inputs.forEach((input) => {
        ...
        if (
          ...
          if (Array.isArray(value))
            ...
            fileIsUploading =
              value.transferMethod === TransferMethod.local_file &&
              !value.uploadedId;
  ...
  return (
    <div
      className="absolute inset-0 z-10 rounded-[4px] pt-10"
      style={{
        backgroundColor: 'rgba(16, 24, 40, 0.20)'
      }}
    >
      <div className="flex h-full flex-col rounded-[4px] bg-white">
            ...
            <div className="mt-4 flex justify-between space-x-2 px-4">
              {isRunning && (
                <div
                  ...
                  onClick={onStop}
                >
                  <RiStopCircleLine className="h-4 w-4 text-gray-500" />
                </div>
              )}
              <Button
                disabled={!isFileLoaded || isRunning}
                variant="primary"
                className="custom-primary w-0 grow space-x-2"
                onClick={handleRun}
              >
                {isRunning && (
