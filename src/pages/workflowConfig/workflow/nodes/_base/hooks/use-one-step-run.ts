...
const useOneStepRun = <T>({
  ...
  const getVar = (valueSelector: ValueSelector): Var | undefined => {
    ...
    const targetVar = allOutputVars.find((item) =>
      isSystem ? !!item.isStartNode : item.nodeId === valueSelector[0]
    );
    ...
    for (let i = 1; i < valueSelector.length; i++) {
      ...
      if (Array.isArray(curr))
        curr = curr.find(
          (v: any) => v.variable.replace('conversation.', '') === key
        );
  ...
  const handleRun = (submitData: Record<string, any>) => {
    ...
    try {
      if (!isIteration && !isLoop) {
        ...
        const getIterationSingleNodeRunUrl = (...args: any) => {
          return '';
        };
        ssePost(
        ...
        const ssePost = (...args: any) => {};
        const getLoopSingleNodeRunUrl = (...args: any) => {
          return '';
        };
        ssePost(
          ...
          { body: { inputs: submitData } },
          {
            ...
            onLoopFinish: (params) => {
              ...
              const currentIndex = loopRunResult.findIndex(
                (trace) => trace.id === params.data.id
              );
