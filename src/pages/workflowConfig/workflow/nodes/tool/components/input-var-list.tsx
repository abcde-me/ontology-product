...
import { FormTypeEnum } from '@/pages/workflowConfig/models/model';
import VarReferencePicker from '@/pages/workflowConfig/workflow/nodes/_base/components/variable/var-reference-picker';
  ...
  const { availableVars, availableNodesWithParent } = useAvailableVarList(
    nodeId,
    {
      ...
      filterVar: (varPayload: Var) => {
        return [VarType.string, VarType.number, VarType.secret].includes(
          varPayload.type
        );
      }
    }
  ...
  return (
    <div className="space-y-3">
      {schema.map((schema, index) => {
        ...
        const isFile =
          type === FormTypeEnum.file || type === FormTypeEnum.files;
        ...
        return (
          <div key={variable} className="space-y-1">
            <div className="flex h-[18px] items-center space-x-2">
              <span className="code-sm-semibold text-text-secondary">
              ...
              {required && (
                <span className="system-xs-regular text-util-colors-orange-dark-orange-dark-600">
                  Required
                </span>
              )}
            </div>
              ...
              <Input
                className={cn(
                  inputsIsFocus[variable]
                    ? 'border-gray-300 bg-gray-50 shadow-xs'
            ...
            {(isNumber || isSelect) && (
              <VarReferencePicker
                ...
                defaultVarKindType={
                  varInput?.type ||
                  (isNumber ? VarKindType.constant : VarKindType.variable)
                }
