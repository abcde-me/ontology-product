...
function addTitle(
  ...
  list.forEach((node) => {
    const parallel_id =
      node.parallel_id ?? node.execution_metadata?.parallel_id ?? null;
    ...
    if (
      node.parallelDetail?.children &&
      node.parallelDetail.children.length > 0
    ) {
      addTitle(
        {
...
const format = (
  ...
  result.forEach((node) => {
    ...
    const parent_parallel_id =
      node.parent_parallel_id ??
      node.execution_metadata?.parent_parallel_id ??
      null;
    ...
    if (isParallelStartNode) {
      ...
      if (parentParallelStartNode) {
        ...
        if (parentParallelStartNode.parallelDetail.children) {
          const sameBranchNodesLastIndex = (
            parentParallelStartNode.parallelDetail.children as any
          ).findLastIndex((node) => {
    ...
    const parallelStartNode = result.find(
      (item) => item.node_id === parallelFirstNodeMap[parallel_id]
    );
    ...
    if (
      ...
      const sameBranchNodesLastIndex = (
        parallelStartNode.parallelDetail.children as any
      ).findLastIndex((node) => {
        const currStartNodeId =
          node.parallel_start_node_id ??
          node.execution_metadata?.parallel_start_node_id ??
      ...
      if (sameBranchNodesLastIndex !== -1) {
        parallelStartNode.parallelDetail.children.splice(
          sameBranchNodesLastIndex + 1,
          0,
          node
        );
