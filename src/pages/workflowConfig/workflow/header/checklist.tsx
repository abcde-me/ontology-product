  ...
  return (
    ...
    >
      <PortalToFollowElemTrigger onClick={() => !disabled && setOpen(v => !v)}>
        <div
          className={cn(
            'relative ml-0.5 flex items-center justify-center w-7 h-7 rounded-[4px]',
            disabled && 'opacity-50 cursor-not-allowed',
          )}
        >
          <div
            className={cn('group flex items-center justify-center w-full h-full rounded-[4px] cursor-pointer')}
          >
            <RiListCheck3
              className={cn('w-4 h-4', 'text-components-button-ghost-text')}
            />
          </div>
          {
            !!needWarningNodes.length && (
              <div className='absolute -right-3 -top-2.5 flex items-center justify-center min-w-[18px] h-[18px] rounded-full border border-gray-100 text-white text-[11px] font-semibold bg-[#F79009]'>
                {needWarningNodes.length}
              </div>
            )
          }
      ...
      <PortalToFollowElemContent className='z-[12]'>
        <div
          className='w-[420px] rounded-[4px] bg-white border-[0.5px] border-black/5 shadow-lg overflow-y-auto'
          style={{
                  ...
                  <div className='px-4 py-2'>
                    {
                      needWarningNodes.map(node => (
                        <div
                          key={node.id}
                          className='mb-2 last-of-type:mb-0 border-[0.5px] border-gray-200 bg-white shadow-xs rounded-[4px] cursor-pointer'
                          onClick={() => {
                        ...
                        >
                          ...
                          <div className='border-t-[0.5px] border-t-black/2'>
                            {
                              node.unConnected && (
                                <div className='px-3 py-2 bg-gray-25 rounded-b-[4px]'>
                                  <div className='flex text-xs leading-[18px] text-gray-500'>
                                    <RiAlertFill className='mt-[3px] mr-2 w-3 h-3 text-[#F79009]' />
