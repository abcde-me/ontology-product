...
// import { useKnowledge } from '@/pages/workflowConfig/hooks/use-knowledge'
import cn from '@/pages/workflowConfig/utils/classnames'
// import datasetsJson from '@/pages/workflowConfig/mockData/datasets.json'
...
const SelectDataSet: FC<ISelectDataSetProps> = ({
  ...
  useInfiniteScroll(
    async () => {
      if (!isNoMore) {
        const result = await getDatasetsList({page})
        const data = result.data.data
        const has_more = result.limit * result.page < result.total
        setPage(getPage() + 1)
    ...
    >
      ...
      {(loaded && hasNoData) && (
        ...
        >
          ...
          <a href="/datasets/create" className='font-normal text-text-accent'>{t('appDebug.feature.dataSet.toCreate')}</a>
        </div>
      )}
          ...
          <div ref={listRef} className='mt-7 space-y-1 max-h-[286px] overflow-y-auto'>
            {datasets.map(item => (
              <div
                key={item.id}
                className={cn(
                  'flex justify-between items-center h-10 px-2 rounded-[4px] bg-components-panel-on-panel-item-bg border-components-panel-border-subtle border-[0.5px] shadow-xs cursor-pointer hover:border-components-panel-border hover:bg-components-panel-on-panel-item-bg-hover hover:shadow-sm',
              ...
              >
                <div className='mr-1 flex items-center overflow-hidden'>
                  <div className={cn('mr-2', !item.embedding_available && 'opacity-30')}>
                    <KbIcon className='w-4 h-4 text-[#444CE7]' />
                  </div>
