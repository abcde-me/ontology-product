...
import type { ModelParameterRule } from '@/pages/workflowConfig/models/model'
...
import Radio from '@/pages/workflowConfig/components/radio'
import { SimpleSelect } from '@/pages/workflowConfig/components/select'
...
const ParameterItem: FC<ParameterItemProps> = ({
  ...
  const renderInput = () => {
    ...
    if (parameterRule.type === 'int') {
      ...
      return (
        <>
          ...
          <input
            ref={numberInputRef}
            className='shrink-0 block ml-4 pl-3 w-16 h-8 appearance-none outline-none rounded-[4px] bg-components-input-bg-normal text-components-input-text-filled system-sm-regular'
            type='number'
            max={parameterRule.max}
            min={parameterRule.min}
    ...
    if (parameterRule.type === 'string' && !parameterRule.options?.length) {
      return (
        <input
          className={cn(isInWorkflow ? 'w-[178px]' : 'w-full', 'ml-4 flex items-center px-3 h-8 appearance-none outline-none rounded-[4px] bg-components-input-bg-normal text-components-input-text-filled system-sm-regular')}
    ...
    if (parameterRule.type === 'text') {
      return (
        <textarea
          className='w-full h-20 ml-4 px-1 rounded-[4px] bg-components-input-bg-normal text-components-input-text-filled system-sm-regular'
          value={renderValue as string}
          onChange={handleStringInputChange}
        />
      )
