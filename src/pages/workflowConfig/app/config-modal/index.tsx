...
import { checkKeys, getNewVarInWorkflow } from '@/pages/workflowConfig/utils/var'
import ConfigContext from '@/pages/workflowConfig/context/debug-configuration'
import type { InputVar, MoreInfo, UploadFileSetting } from '@/pages/workflowConfig/workflow/types'
import Modal from '@/pages/workflowConfig/components/modal'
import { ChangeType, InputVarType, SupportUploadFileTypes } from '@/pages/workflowConfig/workflow/types'
import FileUploadSetting from '@/pages/workflowConfig/workflow/nodes/_base/components/file-upload-setting'
import Checkbox from '@/pages/workflowConfig/components/checkbox'
import { DEFAULT_FILE_UPLOAD_SETTING } from '@/pages/workflowConfig/workflow/constants'
...
const ConfigModal: FC<IConfigModalProps> = ({
  ...
  const [tempPayload, setTempPayload] = useState<InputVar>(
    Object.assign({}, payload || getNewVarInWorkflow(''), {allowed_file_upload_methods: [TransferMethod.local_file]})
  )
  ...
  return (
    <Modal
      ...
      onClose={onClose}
    >
      <div className='mb-8 mt-4' ref={modalRef} tabIndex={-1}>
        <div className='space-y-4'>
          <Field title={t('appDebug.variableConfig.fieldType')}>
            <div className='grid grid-cols-3 gap-2'>
              <SelectTypeItem type={InputVarType.textInput} typeDesc="String" selected={type === InputVarType.textInput} onClick={handleTypeChange(InputVarType.textInput)} />
              <SelectTypeItem type={InputVarType.paragraph} typeDesc="Text" selected={type === InputVarType.paragraph} onClick={handleTypeChange(InputVarType.paragraph)} />
              <SelectTypeItem type={InputVarType.select} typeDesc="Select" selected={type === InputVarType.select} onClick={handleTypeChange(InputVarType.select)} />
              <SelectTypeItem type={InputVarType.number} typeDesc="Number" selected={type === InputVarType.number} onClick={handleTypeChange(InputVarType.number)} />
              {supportFile && <>
                <SelectTypeItem type={InputVarType.singleFile} typeDesc="File" selected={type === InputVarType.singleFile} onClick={handleTypeChange(InputVarType.singleFile)} />
                <SelectTypeItem type={InputVarType.multiFiles} typeDesc="Files" selected={type === InputVarType.multiFiles} onClick={handleTypeChange(InputVarType.multiFiles)} />
              </>}
            </div>
          </Field>
