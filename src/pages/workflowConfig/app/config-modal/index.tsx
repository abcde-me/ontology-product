...
import {
  checkKeys,
  getNewVarInWorkflow
} from '@/pages/workflowConfig/utils/var';
...
const ConfigModal: FC<IConfigModalProps> = ({
  ...
  const [tempPayload, setTempPayload] = useState<InputVar>(
    Object.assign({}, payload || getNewVarInWorkflow(''), {
  ...
  return (
    <Modal
      title={t(
        `appDebug.variableConfig.${isCreate ? 'addModalTitle' : 'editModalTitle'}`
      )}
      isShow={isShow}
      onClose={onClose}
    >
      <div className="mb-8 mt-4" ref={modalRef} tabIndex={-1}>
        <div className="space-y-4">
          ...
          {[InputVarType.singleFile, InputVarType.multiFiles].includes(
            type
          ) && (
            <FileUploadSetting
          ...
          <div className="!mt-5 flex h-6 items-center space-x-2">
            ...
            <span className="system-sm-semibold text-text-secondary">
              {t('appDebug.variableConfig.required')}
            </span>
          </div>
        </div>
      </div>
