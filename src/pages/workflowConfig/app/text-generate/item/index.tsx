...
// import { fetchMoreLikeThis, updateFeedback } from '@/pages/workflowConfig/service/share'
// import { fetchTextGenerationMessage } from '@/pages/workflowConfig/service/debug'
import { useStore as useAppStore } from '@/pages/workflowConfig/app/store';
import WorkflowProcessItem from '@/pages/workflowConfig/chat/chat/answer/workflow-process';
...
import { useChatContext } from '@/pages/workflowConfig/chat/chat/context';
import ActionButton, {
  ActionButtonState
...
const GenerationItem: FC<IGenerationItemProps> = ({
  ...
  const handleFeedback = (childFeedback: FeedbackType) => {
    ...
    console.warn('API NOT IMPLEMENTED', 'updateFeedback');
  };
  ...
  const handleMoreLikeThis = () => {
    ...
    console.warn('API NOT IMPLEMENTED', 'fetchMoreLikeThis');
    const res: any = {};
  ...
  const handleOpenLogModal = () => {
    ...
    //   messageId: messageId!,
    // })
    console.warn('API NOT IMPLEMENTED', 'fetchTextGenerationMessage');
    const data = {} as any;
    const logItem = {
      ...data,
      log: [
  ...
  const showResultTabs =
    !!workflowProcessData?.resultText || !!workflowProcessData?.files?.length;
  ...
  return (
    <>
      <div className={cn('relative', !isTop && 'mt-3', className)}>
        {isLoading && (
          <div className={cn('flex h-10 items-center', !inSidePanel && '')}>
            <Loading type="area" />
          </div>
        )}
        {!isLoading && (
          <>
            {/* result content */}
            <div className={cn('relative', !inSidePanel && '')}>
              {workflowProcessData && (
                <>
                  <div
                    className={cn(
                  ...
                  >
                    {taskId && (
                      ...
                      >
                        <RiArrowRightBoxFill className="mr-1 h-3 w-3" />
                    ...
                    {showResultTabs && (
                      <div className="result-tabs flex items-center space-x-6 px-1">
                        <div
                          className={cn(
              ...
              {!workflowProcessData && taskId && (
                <div
                  className={cn(
                    'system-2xs-medium-uppercase sticky left-0 top-0 flex w-full items-center rounded-t-2xl bg-components-actionbar-bg p-4 pb-3 text-text-accent-secondary',
                    isError && 'text-text-destructive'
                  )}
                >
              ...
              {isError && (
                <div className="body-lg-regular p-4 pt-0 text-text-quaternary">
            ...
            >
              ...
              {/* action buttons */}
              <div className="action-btns absolute bottom-1 right-2 flex items-center">
