          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className={classNames(
                'w-full max-w-[480px] transform rounded-[4px] bg-components-panel-bg px-[28px] py-[20px] text-left align-middle shadow-xl transition-all',
                overflowVisible ? 'overflow-visible' : 'overflow-hidden',
                className,
              )}>
                {title && <Dialog.Title
                  as="h3"
                  className="text-text-primary text-[16px]/[24px] font-semibold text-[#0F172A]"
                >
                  {title}
                </Dialog.Title>}
                {description && <Dialog.Description className='text-text-secondary body-md-regular mt-[20px]'>
                  {description}
                </Dialog.Description>}
                {closable
