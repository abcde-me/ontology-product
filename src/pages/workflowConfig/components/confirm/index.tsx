  ...
  return createPortal(
    <div className={'fixed inset-0 flex items-center justify-center z-[10000000] bg-background-overlay'}
      ...
      <div ref={dialogRef} className={'relative w-full max-w-[480px] overflow-hidden'}>
        <div className='flex flex-col items-start max-w-full rounded-[4px] border-[0.5px] border-solid border-components-panel-border shadows-shadow-lg bg-components-panel-bg'>
          <div className='flex pt-6 pl-6 pr-6 pb-4 flex-col items-start gap-2 self-stretch'>
            <div className='title-2xl-semi-bold text-text-primary modal-title'>{title}</div>
          ...
          <div className='flex p-6 gap-2 justify-end items-start self-stretch'>
            {showCancel && <Button onClick={onCancel} className="rounded-[4px] min-w-[80px]">{cancelTxt}</Button>}
            {showConfirm && <Button variant={'primary'} className="rounded-[4px] min-w-[80px]" destructive={type !== 'info'} loading={isLoading} disabled={isDisabled} onClick={onConfirm}>{confirmTxt}</Button>}
          </div>
        </div>
      </div>
    </div>, document.body,
