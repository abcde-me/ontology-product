...
const TabHeader: FC<ITabHeaderProps> = ({
  ...
  const renderItem = ({ id, name, icon, extra }: Item) => (
    <div
      ...
      className={cn(
        ...
        id === value ? 'text-text-primary border-components-tab-active active' : 'text-text-tertiary',
      )}
      onClick={() => onChange(id)}
    >
      {icon || ''}
  ...
  return (
    <div className='flex justify-between tab-header'>
      <div className='flex space-x-4 tab-header-item'>
