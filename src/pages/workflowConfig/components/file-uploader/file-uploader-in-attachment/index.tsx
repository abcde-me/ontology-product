...
const FileUploaderInAttachment = ({
  ...
  const options = [
    {
      ...
      label: '本地文件', //t('common.fileUploader.uploadFromComputer'),
      icon: null //<RiUploadCloud2Line className='w-4 h-4' />,
    }
    // {
    //   value: TransferMethod.remote_url,
    //   label: "链接地址",// t('common.fileUploader.pasteFileLink'),
    //   icon: null// <RiLink className='w-4 h-4' />,
    // },
  ];
    ...
    (option: Option, open?: boolean) => {
      return (
        <Button
          ...
          variant="tertiary"
          className={cn(
            'relative grow rounded-[4px]',
            open && 'bg-components-button-tertiary-bg-hover'
          )}
          disabled={
            !!(
              fileConfig.number_limits &&
              files.length >= fileConfig.number_limits
            )
          }
        >
          {option.icon}
  ...
  const renderOption = useCallback(
    (option: Option) => {
      if (
        option.value === TransferMethod.local_file &&
        fileConfig?.allowed_file_upload_methods?.includes(
