...
export const getFileAppearanceType = (
  fileName: string,
  fileMimetype: string
) => {
...
export const isAllowedFileExtension = (
  ...
  return getSupportFileExtensionList(
    allowFileTypes,
    allowFileExtensions
  ).includes(getFileExtension(fileName, fileMimetype).toUpperCase());
};
  ...
  const result = Object.keys(rawData || {})
    .map((key) => {
      if (
        ...
        rawData[key]?.dify_model_identity === '__dify__file__'
      ) {
        return {
          varName: key,
      ...
      if (
        ...
        rawData[key].some(
          (item) => item?.dify_model_identity === '__dify__file__'
        )
      ) {
        return {
          varName: key,
...
export const fileIsUploaded = (file: FileEntity) => {
  ...
  if (
    file.transferMethod === TransferMethod.remote_url &&
    file.progress === 100
  )
    return true;
};
