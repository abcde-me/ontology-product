...
import type { Credential, CustomCollectionBackend, CustomParamSchema } from '../types'
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn('API NOT IMPLEMENTED', 'parseParamsSchema')
        const parameters_schema: any = []
        const schema_type: any = {}
  ...
  return (
    <>
      <Drawer
        ...
        panelClassName='mt-[50px] !mr-0 mb-0 rounded-none !w-[640px]'
        ...
        body={
          <div className='flex flex-col h-full'>
            <div className='grow h-0 overflow-y-auto px-6 py-3 space-y-4'>
              <div>
                <div className='py-2 system-sm-medium text-text-primary'><span className='mr-1 text-red-500'>*</span> {t('tools.createTool.name')}</div>
                <div className='flex items-center justify-between gap-3 tool-icon'>
                  <AppIcon size='large' onClick={() => {}} className='cursor-pointer' icon={emoji.content} background={emoji.background} />
                  <Input
                    className='h-10 grow rounded-[4px]' placeholder={t('tools.createTool.toolNamePlaceHolder')!}
              ...
              <div className='select-none'>
                <div className='flex justify-between items-center'>
                  <div className='flex items-center'>
                    <div className='py-2 system-sm-medium text-text-primary'><span className='mr-1 text-red-500'>*</span> {t('tools.createTool.schema')}</div>
                    ...
                    <Tooltip
                      popupContent="https://swagger.io/specification/"
                    >
                      <div className='text-xs font-normal'>{t('tools.createTool.viewSchemaSpec')}</div>
                    </Tooltip>
                  </div>
                ...
                <Textarea
                  className='h-[240px] resize-none rounded-[4px]'
                  value={schema}
            ...
            <div className={cn(isEdit ? 'justify-between' : 'justify-end', 'mt-2 shrink-0 flex py-4 px-6 rounded-b-[10px] bg-background-section-burn border-t border-divider-regular')} >
              ...
              <div className='flex space-x-2 '>
                ...
                <Button variant='primary' className="custom-primary" onClick={handleSave}>{t('common.operation.save')}</Button>
              </div>
            </div>
