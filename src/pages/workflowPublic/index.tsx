...
const TextGeneration: FC<IMainProps> = ({
  ...
  const queryAppId = useParams('id') as string;
  ...
  const handleRemoveSavedMessage = (messageId: string) => {
  ...
  useEffect(() => {
    if (siteInfo?.title) {
      ...
      else document.title = `${siteInfo.title} - Powered by AppForge`;
    }
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'text-generation-left relative flex h-full shrink-0 flex-col',
          isPC
            ? 'w-[500px] max-w-[50%]'
            : resultExisted
              ? 'h-[calc(100%_-_64px)]'
        ...
        <div className={cn('shrink-0 space-y-4', isPC ? '' : 'p-4 pb-0')}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-y-[2px]">
              <div className="system-md-semibold grow truncate text-[16px]/[24px] text-[#1E293B] text-text-secondary">
                {siteInfo.title}
              </div>
