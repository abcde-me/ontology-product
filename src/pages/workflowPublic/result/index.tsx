...
import {
  ...
  sendWorkflowMessage
} from '@/pages/workflowConfig/service/share';
...
const Result: FC<IResultProps> = ({
  ...
  const [workflowProcessData, doSetWorkflowProcessData] =
      ...
      sendWorkflowMessage(
        data,
        {
          ...
          onIterationStart: ({ data }) => {
            setWorkflowProcessData(
              produce(getWorkflowProcessData()!, (draft) => {
                draft.expand = true;
                draft.tracing.push({
                  ...data,
          ...
          onIterationNext: () => {
            setWorkflowProcessData(
              produce(getWorkflowProcessData()!, (draft) => {
                draft.expand = true;
                const iterations = draft.tracing.find(
                  (item) =>
          ...
          onIterationFinish: ({ data }) => {
            setWorkflowProcessData(
              produce(getWorkflowProcessData()!, (draft) => {
                draft.expand = true;
                const iterationsIndex = draft.tracing.findIndex(
                  (item) =>
                    item.node_id === data.node_id &&
                    (item.execution_metadata?.parallel_id ===
                      data.execution_metadata?.parallel_id ||
          ...
          onLoopNext: () => {
            setWorkflowProcessData(
              produce(getWorkflowProcessData()!, (draft) => {
                draft.expand = true;
                const loops = draft.tracing.find(
                  (item) =>
                    item.node_id === data.node_id &&
                    (item.execution_metadata?.parallel_id ===
                      data.execution_metadata?.parallel_id ||
          ...
          onLoopFinish: ({ data }) => {
            setWorkflowProcessData(
              produce(getWorkflowProcessData()!, (draft) => {
                draft.expand = true;
                const loopsIndex = draft.tracing.findIndex(
                  (item) =>
                    item.node_id === data.node_id &&
                    (item.execution_metadata?.parallel_id ===
                      data.execution_metadata?.parallel_id ||
          ...
          onWorkflowFinished: ({ data }) => {
            if (isTimeout) {
              notify({
                type: 'warning',
                message: t('appDebug.warningMessage.timeoutExceeded')
              });
              return;
            }
            if (data.error) {
