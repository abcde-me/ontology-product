...
export const getTokenExpiration = (token: string): number | null => {
  try {
    ...
    // 如果exp值大于9999999999（10位数），说明已经是毫秒时间戳
    // 如果exp值小于等于9999999999，说明是秒时间戳，需要乘以1000
    const isMilliseconds = decoded.exp > 9999999999;
    const expTime = isMilliseconds ? decoded.exp : decoded.exp * 1000;
