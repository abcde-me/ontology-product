...
export async function getModelsGroupByProvider(params: any = {}) {
  ...
  Object.keys(modelsListMap).forEach(providerName => {
    modelsList.push({
      ...pick(modelsListMap[providerName][0], ['provider_id', 'provider_name', 'provider_icon_base64']),
      models: modelsListMap[providerName].map(p => omit(p, ['provider_id', 'provider_name', 'provider_icon_base64']))
    })
  })
