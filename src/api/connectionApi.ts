...
export async function delconnectionList(connector_id){
    return await UAPI.RES.delconnection({connector_id}).delete().inRegion().do();
}
// 查看的api
...
export async function updataConnectionList(params:any){
    return await UAPI.RES.editconnection({connector_id:params.connector_id}).put(params.newfrom).inRegion().do();
}