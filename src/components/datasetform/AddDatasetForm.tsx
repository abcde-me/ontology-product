import { Form, Input, Button, Select, Space, Radio, Table, Checkbox, Cascader, Typography } from '@arco-design/web-react';
...
function DatasetForm({ onSubmit, onCancel }) {
  ...
  const getConnectorFileInformationfun = (id: string, type: 'jsonl') => {
    ...
    setConnectorFileInformation(transformToSelectOptions(csconnectorFileInformation))
  }//查询指定连接器加载成功的文件信息
  ...
  return (
    <div>
      ...
      >
        ...
        {
          <div style={{ border: '1px solid black', borderRadius: '10px', padding: '0 40px 0px 0px' }}>
            {
              dataSource === 'volume' ? (
                <>
                  <FormItem
                  ...
                  {previewData ? (
                    <div style={{ marginTop: 16 }}>
                      <div style={{ marginBottom: 8, fontWeight: 'bold' }}>数据预览:</div>
                      <Table
