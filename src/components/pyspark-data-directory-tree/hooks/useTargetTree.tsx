...
export interface DstCatalogItemWithPath {
  base_dir?: string;
  children?: {
    volume?: Array<{
      base_dir: string;
...
export const useTargetTree = () => {
  ...
  const getCatalogList = async (
    ...
    return generateDstFullPath(res?.data?.dst ?? [], 'dst');
  };

  // 将数据集列表转换为树节点
  const convertSourceCatalogToTreeNode = useCallback(
    (catalog: DstCatalogItemWithPath): TreeNodeData => {
  ...
  const convertFileToTreeNode = useCallback(
    (file: GetTargetCatalogFileListItem): TreeNodeData => {
      return {
        key: `file-${file.id}`,
        id: file.id,
        title: file.FileName,
      ...
      return new Promise(async (resolve) => {
        try {
          ...
          if (res.data?.list?.length > 0) {
            // 更新已加载的子节点数据
            const children = res.data.list.map((file) =>
